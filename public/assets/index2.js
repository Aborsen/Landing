/* =========================================================================
   index2.js — page behaviors + showcase engine
   Modules:
     1. Navbar mega-menu dropdowns
     2. Mobile nav drawer
     3. Hero: rotating placeholder + click-to-activate textarea
     4. Accordion (FAQ) — single-open
     5. Stats counter (on viewport enter)
     6. Showcase engine (verbatim port from index.html's imperative JS)
     7. DOM ready wiring
   ========================================================================= */

/* ---------- 1. Navbar dropdowns --------------------------------------- */
(function(){
  var navbar = document.querySelector('.ins-navbar');
  var wrap   = document.querySelector('.page-navbar-wrap');
  if (!navbar || !wrap) return;
  var triggers = navbar.querySelectorAll('[data-dropdown]');
  var menus    = wrap.querySelectorAll('[data-menu]');
  var current  = null;
  ...
  function open(name){
    current = name;
    navbar.classList.add('is-dropdown-open');
    triggers.forEach(function(t){
      t.setAttribute('aria-expanded', t.dataset.dropdown === name ? 'true' : 'false');
    });
    menus.forEach(function(m){ m.hidden = m.dataset.menu !== name; });
  }

  triggers.forEach(function(t){
    t.addEventListener('click', function(e){
      e.stopPropagation();
      var name = t.dataset.dropdown;
      if (current === name) closeAll(); else open(name);
    });
  });
...
(function(){
  ...
  toggle.addEventListener('click', function(){
    wrap.classList.toggle('is-mobile-open');
  });
})();

/* ---------- 3. Hero rotating placeholder + click-to-focus -------------- */
(function(){
  var area        = document.querySelector('[data-hero-chat]');
  var placeholder = document.querySelector('[data-hero-placeholder]');
  var typing      = document.querySelector('[data-hero-typing]');
  var textarea    = document.querySelector('[data-hero-input]');
  if (!area || !placeholder || !typing || !textarea) return;
  ...
  var q = 0, txt = '', deleting = false, active = false, timer = null;

  function tick(){
    if (active) return;
    var full = questions[q];
    if (!deleting && txt.length < full.length) {
      ...
      timer = setTimeout(function(){ deleting = true; tick(); }, 2000);
...
(function(){
  ...
  accs.forEach(function(acc){
    var items = acc.querySelectorAll('.ins-accordion__item');
    items.forEach(function(item){
      var trigger = item.querySelector('.ins-accordion__trigger');
      if (!trigger) return;
      trigger.addEventListener('click', function(){
        var open = item.getAttribute('aria-expanded') === 'true';
        items.forEach(function(x){ x.setAttribute('aria-expanded','false'); });
...
/* ---------- 5. Animated stat counters (ease-out cubic on viewport enter) --- */
(function(){
...
/* =========================================================================
   6. Showcase engine — ported verbatim from original imperative JS.
      All globals (goToStep, p0go, p0connect, p1_*, p2_*, p3_*, p4_*)
      are assigned to window so inline onclick= handlers in the template
      can call them.
   ========================================================================= */
...
/* ---------- 7. Showcase init on DOM ready ---------- */
if (document.readyState === 'loading') { document.addEventListener('DOMContentLoaded', function(){ if (typeof initShowcase === 'function') initShowcase(); }); } else { if (typeof initShowcase === 'function') initShowcase(); }
